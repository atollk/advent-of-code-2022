...
pub fn day_4() {
    ...
    let elf_pairs = file_lines
        ...
        .map(|line| {
            ...
            ((sec1l.parse::<i32>().unwrap(), sec1r.parse::<i32>().unwrap()), (sec2l.parse::<i32>().unwrap(), sec2r.parse::<i32>().unwrap()))
        })
        .collect::<Vec<_>>();
    let pair_overlaps = elf_pairs
        .iter()
        .map(|((sec1l, sec1r), (sec2l, sec2r))| {
            let overlaps_completely = (sec1l <= sec2l && sec1r >= sec2r) || (sec1l >= sec2l && sec1r <= sec2r);
            let overlaps_somewhere = (sec1l <= sec2l && sec2l <= sec1r) || (sec2l <= sec1l && sec1l <= sec2r);
